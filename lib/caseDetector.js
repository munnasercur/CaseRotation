...
const COMMANDS = new Set(["/status", "/undo", "/summary", "/help", "/allcase", "/todaycase", "/announce"]);
...
// ---- Discharge detection: "3 มาโนชญ d/c" (name required), or
// "3 มาโนชญ d/c 5 สมศรี d/c" for multiple ----
// Each group is: a bed number, a REQUIRED name, then a "d/c" (or "dc")
// token that closes the group. The name is required (not "3 d/c" alone)
// because bed numbers get reused over time as patients are discharged
// and readmitted — without the name, a bare bed number could discharge
// the wrong patient. "d/c" is an unambiguous stop marker, so we can
// safely require the ENTIRE message to be made of nothing but these
// groups — any leftover token anywhere (leading, trailing, or between
// groups) rejects the whole message.
...
function detectDischarges(rawText) {
  ...
  while (i < tokens.length) {
    ...
    if (nameTokens.length === 0) return []; // name is required
    if (i >= tokens.length || !DISCHARGE_TOKEN.test(tokens[i])) return []; // group never closed with "d/c"

    results.push({ bedNumber, patientName: nameTokens.join(" ").trim() });
